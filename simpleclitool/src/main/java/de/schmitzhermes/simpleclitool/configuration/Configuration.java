...
public class Configuration {
	...
	public void registerConfigurationItem(ConfigurationItem<?> item) {
		if (configurationItemsPerOptionName.get(item.optionName) != null) {
			throw new IllegalArgumentException("Eine Option mit dem Namen \"" + item.optionName
					+ "\" ist bereits registriert.");
		}
		if (configurationItemsPerArgumentNumber.get(item.argNumber) != null) {
			throw new IllegalArgumentException("Eine Option mit der Argumentennummer \""
					+ item.argNumber + "\" ist bereits registriert.");
		}
